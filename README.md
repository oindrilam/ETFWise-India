# ETFWise India - AI ETF Comparison Assistant

## Purpose

AI assistant to compare Indian ETFs using official/public data sources.

## MVP Categories

- NIFTY_50
- SENSEX
- BANK
- GOLD
- SILVER

## Current Phase

**Phase 1 - Data Design** (complete). **Phase 1A - Market Comparator Analysis** (complete). Phase 2 not started.

## Product Differentiation

- [Market Comparator Analysis](docs/product/market_comparator_analysis.md)

## Documentation

- Architecture: [docs/architecture/ARCHITECTURE.md](docs/architecture/ARCHITECTURE.md)
- Phase documents: [docs/phases/](docs/phases/)
- Data design documents: [docs/data/](docs/data/)

## Disclaimer

Educational comparison only, not investment advice.
